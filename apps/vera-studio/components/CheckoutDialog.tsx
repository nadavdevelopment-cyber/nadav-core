...
export function CheckoutDialog({lines, deliveryFee, onClose, onComplete}: {lines: CartLine[]; deliveryFee: number; onClose: () => void; onComplete: (data: CheckoutData) => Promise<void> | void}) {
  const [data, setData] = useState(initial);
  const [step, setStep] = useState<1 | 2>(1);
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');
  const subtotal = lines.reduce((sum, line) => sum + line.product.price * line.quantity, 0);
  const shipping = data.fulfillment === 'delivery' ? deliveryFee : 0;
    ...
    <section className="checkout" role="dialog" aria-modal="true" aria-labelledby="checkout-title">
      ...
      <div className="checkout__content">
        <div className="checkout__form">
          ...
          </> : <section><h3>¿Cómo querés pagar?</h3><div className="choice-grid payment-choices"><label className="selected"><input type="radio" name="payment" checked readOnly/><strong>Transferencia</strong><small>Al confirmar, el comercio coordina los datos de pago por WhatsApp.</small></label></div><div className="studio-note"><span>V</span><p>El pedido queda pendiente hasta que el comercio confirme la transferencia.</p></div></section>}
        </div>
