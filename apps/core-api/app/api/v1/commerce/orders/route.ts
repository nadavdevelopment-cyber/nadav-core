...
import {asObject, clientAddress, commercePlaceOrder, commerceStoreBySlug, errorResponse, rpc, readJson} from '@nadav/adapters';
import {CoreError, isUuid, type CommerceCheckoutInput} from '@nadav/core';
...
export async function POST(request: Request) {
  try {
    ...
    if (!isUuid(key)) throw new CoreError('INVALID_IDEMPOTENCY_KEY', 'Falta una clave de idempotencia válida.', 400);
