...
export function paymentPreferenceBody(order: Order, restaurantSlug: string, origin: string) {
  ...
  return {items: [{id: order.id, title: `Pedido #${order.number}`, quantity: 1, currency_id: 'ARS', unit_price: order.total}], external_reference: paymentReference(order.restaurantId, order.id), notification_url: `${origin}/api/v1/payments/mercadopago/webhook`, back_urls: {success: `${origin}/orders/${order.id}?restaurant=${restaurantSlug}`, pending: `${origin}/orders/${order.id}?restaurant=${restaurantSlug}`, failure: `${origin}/orders/${order.id}?restaurant=${restaurantSlug}`}, auto_return: 'approved'};
}
...
async function tokenRequest(body: Record<string, string>) {
  ...
  if (!response.ok) throw new Error('Mercado Pago rejected the authorization.');
...
async function accessToken(restaurantId: string) {
  const rows = await supabase<Connection[]>(`core_mp_connections?restaurant_id=eq.${restaurantId}&select=*`);
  const row = rows[0];
...
export async function ensurePaymentPreference(order: Order, restaurantSlug: string) {
  ...
  const existing = await supabase<{preference_id: string | null; init_point: string | null}[]>(`core_payment_intents?restaurant_id=eq.${order.restaurantId}&order_id=eq.${order.id}&select=preference_id,init_point`);
  if (existing[0]?.preference_id && existing[0].init_point) return existing[0].init_point;
  ...
  if (!reserved.length) throw new Error('Payment preference is already being created.');
  const origin = process.env.APP_ORIGIN?.replace(/\/$/, '');
  if (!origin) throw new Error('APP_ORIGIN is not configured.');
  const body = paymentPreferenceBody(order, restaurantSlug, origin);
  try {
    ...
    await supabase(`core_payment_intents?restaurant_id=eq.${order.restaurantId}&order_id=eq.${order.id}&status=eq.creating`, {method: 'PATCH', prefer: 'return=minimal', body: {merchant_id: merchant, preference_id: data.id, init_point: url.toString(), status: 'ready', updated_at: new Date().toISOString()}});
    return url.toString();
  } catch (error) {
    await supabase(`core_payment_intents?restaurant_id=eq.${order.restaurantId}&order_id=eq.${order.id}&status=eq.creating`, {method: 'DELETE', prefer: 'return=minimal'}).catch(() => undefined);
    throw error;
  }
}
...
export async function reconcileMercadoPagoPayment(restaurantId: string, order: Order, payment: Payment) {
  const intent = await supabase<{merchant_id: number; preference_id: string}[]>(`core_payment_intents?restaurant_id=eq.${restaurantId}&order_id=eq.${order.id}&select=merchant_id,preference_id`);
  const expected = intent[0];
  if (!expected || payment.collector_id !== expected.merchant_id || payment.currency_id !== 'ARS' || payment.external_reference !== paymentReference(restaurantId, order.id) || payment.transaction_amount !== order.total || payment.preference_id && payment.preference_id !== expected.preference_id) throw new Error('Payment does not match order.');
