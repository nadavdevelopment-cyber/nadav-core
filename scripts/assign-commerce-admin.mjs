...
const email = process.env.VERA_ADMIN_EMAIL;
const slug = process.env.VERA_RESTAURANT_SLUG ?? 'vera-studio';
if (!base || !service || !email) throw new Error('Definí SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY y VERA_ADMIN_EMAIL.');
...
const response = await fetch(`${base}/rest/v1/core_commerce_members`, {method: 'POST', headers, body: JSON.stringify({restaurant_id: restaurants[0].id, user_id: user.id, role: process.env.VERA_ADMIN_ROLE ?? 'owner'})});
if (!response.ok) throw new Error(`No se pudo asignar el rol (${response.status}).`);
console.log(`Asignado ${email} como ${process.env.VERA_ADMIN_ROLE ?? 'owner'} en ${slug}.`);
